...
//echo "attachment import";
//import_attachment_by_articles();

import_home_images_by_articles();
...
function import_home_images_by_articles(){
 wp_reset_postdata();
$home_post =  query_posts(array(
        'post_status' => 'publish',
...
if (have_posts()) :
    ...
    $attachment_id = $articleId = $attachmentID = $attachment_meta = $fileheaderID =  $fileMasterId = $fileNameOriginal = $fileName = null;
    $post_meta = get_post_meta(get_the_ID());
    $articleId = get_the_ID();
    if(empty($post_meta['post_home-image_thumbnail_id'][0]) && !empty($post_meta['_thumbnail_id'][0])){
        echo "count $i <br>";
        
        
    if(!empty($post_meta['_thumbnail_id'][0])){
        $attachment_id = $post_meta['_thumbnail_id'][0];
    }elseif(!empty($post_meta['header'][0])){
     $attachment_id = $post_meta['header'][0];
    }if(!empty($post_meta['post_flipper-image_thumbnail_id'][0])){
      $attachment_id = $post_meta['post_flipper-image_thumbnail_id'][0];
    }elseif(!empty($post_meta['promoLarge'][0])){
     $attachment_id = $post_meta['promoLarge'][0];
    }
   ...
   $fileMasterId =  explode('_', $attachment_meta->post_name);
   $fileNameOriginal =  end(explode('_', $attachment_meta->post_name));
   if(empty($fileNameOriginal)){
       $fileNameOriginal = $articleId;
   }
   
   if(empty($fileMasterId[0])){
       echo "Fileheader not available <br>";
       //echo $articleId." --- article <br>";
   }else{
       
       $imageDownloadUrlPath =  $fileMasterId[0]."+w288+h140+re0+cr1+ar0/".$fileNameOriginal.".jpg"; 
       $imageDownloadUrl = "http://image.adam.automotive.com/f/".$imageDownloadUrlPath;
       $fileName = $fileMasterId[0]."_home-image_".$fileNameOriginal.".jpg";
       
//       echo "imageDownloadUrl - $imageDownloadUrl <br>";
...
//       continue;
    ...
    $attachmentID = download_attachment($imageDownloadUrl,$fileName,$articleId);
    if(empty($attachmentID)){
        $articleErrors[] = "ERROR: Download attachment failed. url  - $imageDownloadUrl articleId - $articleId  <br>";
    }else if($attachmentID[0]){
             update_post_meta($articleId, "post_home-image_thumbnail_id", $attachmentID[0]);
            echo "$i success -- $attachmentID[0] <br>";
             $i++;
    }
...
//    echo "<pre>";
//   // print_r($attachment_meta->post_name);
//    print_r($fileheaderID[0]);
//    //echo list($part1, $part2) = explode('.', $attachment_meta->post_name);;
//    echo "</pre>";
...
//    if(!empty($post_meta['post_home-image_thumbnail_id']))
//        print_r( $post_meta['post_home-image_thumbnail_id']);
